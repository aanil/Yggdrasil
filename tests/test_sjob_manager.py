...
class TestSlurmJobManager(unittest.TestCase):
    ...
    async def test_monitor_job(self):
        with unittest.mock.patch.object(self.manager, '_job_status', new_callable=AsyncMock) as mock_job_status, \
             unittest.mock.patch.object(self.manager, 'check_status') as mock_check_status:
    ...
    @patch('lib.utils.sjob_manager.asyncio.create_subprocess_exec', new_callable=AsyncMock)
    ...
    def test_submit_job(self, mock_wait_for, mock_create_subprocess_exec):
        # Set up the mocks
        mock_create_subprocess_exec.return_value.communicate.return_value = (b'1234', b'')
    ...
    @patch('lib.utils.sjob_manager.asyncio.create_subprocess_shell', new_callable=AsyncMock)
    @patch('lib.utils.sjob_manager.asyncio.wait_for', new_callable=AsyncMock)
    def test__job_status(self, mock_wait_for, mock_create_subprocess_shell):
        # Set up the mocks
        mock_create_subprocess_shell.return_value.communicate.return_value = (b'COMPLETED', b'')
        ...
        mock_create_subprocess_shell.assert_called_once_with(
            'sacct -n -X -o State -j 1234', stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        mock_wait_for.assert_called_once()
