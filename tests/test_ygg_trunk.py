...
class TestProcessCouchDBChanges(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_process_couchdb_changes(self, mock_logging, mock_sleep, mock_gather, mock_create_task, mock_load_realm_class, mock_fetch_data):
        # Mock the dependencies
        mock_fetch_data.return_value.__aiter__.return_value = iter([(MagicMock(), 'module_loc', 'module_options')])
        mock_load_realm_class.return_value = MagicMock(proceed=True, process=AsyncMock())
