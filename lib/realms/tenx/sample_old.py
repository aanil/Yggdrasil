...
class TenXCompositeSample(TenXSampleBase):
    def __init__(self, sample_id, subsamples, project_info, config, yggdrasil_db_manager):
        """
    ...
    async def process(self):
        ...
        missing_fastq_subsamples = [subsample.sample_id for subsample in self.subsamples if not subsample.fastq_dirs]
    ...
    def generate_libraries_csv(self):
        ...
        with open(library_csv_path, 'w', newline='') as csvfile:
            ...
            for subsample in self.subsamples:
                ...
                if not feature_type:
                    logging.error(f"Feature type not found for feature '{subsample.feature}' in sample '{subsample.sample_id}'")
                    continue
